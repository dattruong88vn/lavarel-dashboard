...
## --------------------------------------------------------
    ...
    public function blurImage($width, $height, $cropPos = 'm')
        # Author:     lamkhtn@gmail.com
        # Date:       08-09-11
        # Purpose:    Blurs the image
        # Param in:   $newWidth: crop with
        #             $newHeight: crop height
        #             $cropPos: Can be any of the following:
        #                   tl, t, tr, l, m, r, bl, b, br, auto
        ...
        #
    {
        ...
        $blurImage = $this->openImage($this->fileName);
        $cropArray = $this->getCropPlacing($this->width, $this->height, $width, $height, $cropPos);
        ...
        imageconvolution($blurImage, $gaussian, 16, 0);
        $blurStep = 1;
        if ($this->width > $this->height) {
            $blurStep = intval($this->width / 1000);
        } else {
            $blurStep = intval($this->height / 1000);
        }
        if ($blurStep == 0) {
            $blurStep = 1;
        }
        $blurStep *= 10;
        for ($i = 0; $i < $blurStep; $i++) {
            imagefilter($blurImage, IMG_FILTER_GAUSSIAN_BLUR);
            imagefilter($blurImage, IMG_FILTER_SMOOTH, -5);
        }
        imagecopymerge($this->image, $blurImage, $cropStartX, $cropStartY, $cropStartX, $cropStartY, $width, $height,
            100);

    }

    public function flipImage($type)
        # Author:     lamkhtn@gmail.com
        # Date:       08-09-11
        # Purpose:    Flips the image
        # Param in:   $type: 1 -> horizontal, 2 -> vertical, 3 -> both
        # Param out:  n/a
        # Reference:
        # Notes:
        #
    {
        ...
        imageflip($this->image, $type);
    }

    public function brightnessImage($brightness)
        # Author:     lamkhtn@gmail.com
        # Date:       08-09-11
        # Purpose:    Flips the image
        # Param in:   $type: 1 -> horizontal, 2 -> vertical, 3 -> both
...
## --------------------------------------------------------
        ...
        #
    {
        ...
        if (in_array($this->fileExtension, $this->transparentArray) && $this->keepTransparency) {
            ...
            $color = imagecolorallocate($im, $this->fillColorArray['r'], $this->fillColorArray['g'],
                $this->fillColorArray['b']);
...
## --------------------------------------------------------
        ...
        #
    {
        ...
        imagecopyresampled($crop, $this->imageResized, 0, 0, $cropStartX, $cropStartY, $newWidth, $newHeight, $newWidth,
            $newHeight);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
    ...
    )
    {
        ...
        if ($inside) {
            ...
            imagecopymerge($final, $this->imageResized, 0, 0, 0, $reflectionHeight, $this->width,
                $this->height - $reflectionHeight, 100);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
        ...
        #
    {
        if ($this->imageResized) {
            ...
            // *** Rotate
            $this->imageResized = imagerotate($this->imageResized, $degrees,
                $bg); // Rotate 45 degrees and allocated the transparent colour as the one to make transparent (obviously)
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
        ...
        #
    {
        ...
        $cornerImg = imagerotate($cornerImg, 90, 0);
        imagecopymerge($this->imageResized, $cornerImg, $this->width - $radius, $this->height - $radius, 0, 0, $radius,
            $radius, 100); #br
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
        ...
        #
    {
        ...
        imagecopymerge($rgb, $newImage, $blurWidth * 0.5 - $distWidth, $blurHeight * 0.5 - $distHeight, 0, 0,
        ...
        for ($i = 0; $i <= STEPS; $i++) {
            ...
            $colour = imagecolorallocate($shadow, $intensity, $intensity, $intensity);
            imagefilledarc($shadow, $blurWidth - 1, $blurHeight - 1, 2 * (1 - $t) * $blurWidth,
                2 * (1 - $t) * $blurHeight, 180, 268, $colour, IMG_ARC_PIE);
            imagefilledarc($shadow, $width, $blurHeight - 1, 2 * (1 - $t) * $blurWidth, 2 * (1 - $t) * $blurHeight, 270,
                358, $colour, IMG_ARC_PIE);
            imagefilledarc($shadow, $width, $height, 2 * (1 - $t) * $blurWidth, 2 * (1 - $t) * $blurHeight, 0, 90,
                $colour, IMG_ARC_PIE);
            imagefilledarc($shadow, $blurWidth - 1, $height, 2 * (1 - $t) * $blurWidth, 2 * (1 - $t) * $blurHeight, 90,
                180, $colour, IMG_ARC_PIE);
        }


        $colour = imagecolorallocate($shadow, 255, 255, 255);
        imagefilledrectangle($shadow, $blurWidth, $blurHeight, $width, $height, $colour);
        imagefilledrectangle($shadow, $blurWidth * 0.5 - $distWidth, $blurHeight * 0.5 - $distHeight,
        ...
        for ($theX = 0; $theX < imagesx($rgb); $theX++) {
            for ($theY = 0; $theY < imagesy($rgb); $theY++) {
                ...
                if (strtolower($bgColor) == 'transparent') {
                    ...
                    $myColour = imagecolorallocate($rgb, $r * (1.0 - $t) + $r0 * $t, $g * (1.0 - $t) + $g0 * $t,
                        $b * (1.0 - $t) + $b0 * $t);
                }
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
        ...
        #
    {
        ...
        imagefilledrectangle($this->imageResized, $positionArray['x1'], $positionArray['y1'], $positionArray['x2'],
            $positionArray['y2'], $transparent);
    }
        ...
        #
    {
        ...
        for ($x = 0; $x < $w; $x++) {
            for ($y = 0; $y < $h; $y++) {
                ...
                $alphacolorxy = imagecolorallocatealpha($img, ($colorxy >> 16) & 0xFF, ($colorxy >> 8) & 0xFF,
                    $colorxy & 0xFF, $alpha);
