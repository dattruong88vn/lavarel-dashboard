class PropzyFileUploadLib {
    ...
    async render() {
        ...
        const items = that._LIST.map((file,index) => {
            let fileName = $.trim(file.fileName) ? file.fileName.split('.')[0] : '';
            let extenstion = $.trim(file.fileName) ? file.fileName.split('.')[1] : '';
        ...
        html = '<div class="col-sm-12 col-md-12 col-xs-12 propzy-upload-file-wrapper">' +
            ...
            '<button class="btn btn-primary btn-sm propzy-upload-file-action-top" data-type="1">Thêm mới</button>' +
