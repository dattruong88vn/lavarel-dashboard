...
$(".reject-reason").click(function() {
    if ($("input[name=reject-reason]:checked").data("control") == "input_text_if_checked") {
        $("#reject-reason-display").show();
    } else {
        $("#reject-reason-display").hide();
    }
});
...
if (json_path != "") {
    ...
    $(json_path).each(function (idx, item) {
        linkArr = item.link.split("/");
        pathConfig.push({
            caption: checkStringNotNull(item.caption) ? item.caption : "",
            width: '120px',
...
$(".file-image-update").fileinput('enable');
...
$(function () {
    ...
    $('#accept-reject').click(function () {
        ...
        if (!$.isNumeric($("input[name=reject-reason]:checked").val())) {
            $('#reject-modal').modal('hide');
            $('#noreason').modal('show');
            return false;
        }
        showPropzyLoading();
        listingObjectBuilding.reasonId = parseInt($("input[name=reject-reason]:checked").val());
        listingObjectNotBuilding.reasonId = listingObjectBuilding.reasonId;
        if (listingObjectBuilding.reasonId == 9) {
            listingObjectBuilding.reasonNote = checkStringNotNull($("#reject-reason-note").val()) ? $("#reject-reason-note").val() : null;
        } else {
            listingObjectBuilding.reasonNote = null;
        }
        fixListing();
        $('#reject-modal').modal('hide');
        updateListing(listingObjectBuilding);
