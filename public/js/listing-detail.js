...
$(function () {
        ...
        get_sync(url_property_type, true, function (data) {
            ...
            if (listingTypeIdNa != 3 || listingTypeIdNa != 4) {
                if ($('#type-business').val() == 2)
                {
                    ...
                    var template = $('#-for-rent-tmpl').html();
                ...
                if ($('#type-business').val() == 1)
                {
                    ...
                    var template = $('#-for-sale-tmpl').html();
                    var compiledTemplate = Template7.compile(template);
                    $('.wrapper-hoahong').html(compiledTemplate({}));
    ...
    // CKEDITOR.replace('noteFromLso');
    // CKEDITOR.replace('noteFromInternal');
...
function prepareListingData() {
    ...
    $(".imageListing .file-preview .file-preview-initial").each(function (idx, element) {
        ...
        listingObjectBuilding.photo.push({
            ...
            caption: checkStringNotNull($(element).find(".file-footer-caption input").val()) ? $(element).find(".file-footer-caption input").val() : null,
    ...
    $('.row-hh').each(function () {
        if ($(this).find('.commision-time').val() != "" && $(this).find('.commision').val() != "" && $('#type-business').val() == 2) {
            ...
            listingObjectNotBuilding.List.push(commision);
            listingObjectNotBuilding.From = null;
            listingObjectNotBuilding.To = null;
            listingObjectNotBuilding.Price = null

            listingObjectBuilding.List.push(commision);
            listingObjectBuilding.From = null;
            listingObjectBuilding.To = null;
            listingObjectBuilding.Price = null
        }
    });
    if ($('.row-hh').find('[name="sell"]:checked').val() == 1 && $('#type-business').val() == 1) {
        if ($('#sellValue').val() != "") {
            listingObjectNotBuilding.From = parseFloat($('.row-hh').find('#sellValue').val());
            listingObjectNotBuilding.To = parseFloat($('.row-hh').find('#sellValue').val());
            listingObjectNotBuilding.Price = null;
            listingObjectNotBuilding.List = null;

            listingObjectBuilding.From = parseFloat($('.row-hh').find('#sellValue').val());
            listingObjectBuilding.To = parseFloat($('.row-hh').find('#sellValue').val());
            listingObjectBuilding.Price = null;
            listingObjectBuilding.List = null;
        } else
        {
            listingObjectNotBuilding.From = null;
            listingObjectNotBuilding.To = null;
    ...
    if ($('.row-hh').find('[name="sell"]:checked').val() == 2 && $('#type-business').val() == 1) {
        if ($('#sellValueMoney').val() != "") {
            listingObjectNotBuilding.From = null;
            listingObjectNotBuilding.To = null;
            listingObjectNotBuilding.Price = parseFloat($('.row-hh').find('#sellValueMoney').val()) * 1000000;
            listingObjectNotBuilding.List = null;

            listingObjectBuilding.From = null;
            listingObjectBuilding.To = null;
            listingObjectBuilding.Price = parseFloat($('.row-hh').find('#sellValueMoney').val()) * 1000000;
            listingObjectBuilding.List = null;
        } else {
            listingObjectNotBuilding.From = null;
            listingObjectNotBuilding.To = null;
    ...
    if (listingObjectNotBuilding.List == null)
    {
        listingObjectNotBuilding.List = null;
        listingObjectBuilding.List = listingObjectNotBuilding.List
    }
    listingObjectBuilding.List = listingObjectNotBuilding.List;
    ...
    listingObjectBuilding.Text = $('#Text').val() == "" ? null : $('#Text').val();
    listingObjectNotBuilding.Text = listingObjectBuilding.Text;
...
var lsDetail = {
  ...
  displaySoldOption: function(soldBy) {
      ...
      let soldOpts = $.grep(options, function(item) {
          return item.source == soldBy;
      })[0].values;

      let soldOptTxt = $(".sold-options-by");
      soldOptTxt.empty();
  ...
  soldListing: function() {
    ...
    var requestData = {
      ...
      soldDate: ($("#sold-date").val()) ? dateStringToTimestamp($("#sold-date").val()) : null,
    ...
    if (typeof errors == "undefined") {
      ...
      $.ajax({
        ...
        if (response.result) {
          ...
          // Freeze button [Sold]
          $("#finish-sold").prop("disabled", true);
        }
        showPropzyAlert(response.message);
      });
    } else {
    ...
    //   });
      showPropzyAlert("Xin vui lòng chọn nguồn");
    }
  },
  rentLs: function() {
  ...
  displayRentOption: function(rentBy) {
    ...
    let rentOpts = $.grep(options, function(item) {
        return item.source == rentBy;
    })[0].values;

    let rentOptTxt = $(".rent-options-by");
    rentOptTxt.empty();
  ...
  rentListing: function() {
    ...
    var constraints = {
      ...
      //   numericality: {
      //     onlyInteger: true,
      //     greaterThan: dateStringToTimestamp($("#rent-from").val()),
      //   },
      // },
      // toDate: {
      //   numericality: {
      //     onlyInteger: true,
      //     greaterThan: dateStringToTimestamp($("#rent-to").val()),
      //   },
      // }
    };
    ...
    if (typeof errors == "undefined") {
        ...
        if ( ($("#rent-from").val()) && ($("#rent-to").val()) ) {
            let rentFromTimestamp = dateStringToTimestamp($("#rent-from").val());
            let rentToTimestamp = dateStringToTimestamp($("#rent-to").val());
        ...
        if (isValidDate) {   
          ...
          $.ajax({
            ...
            if (response.result) {
              ...
              $("#finish-rent").prop("disabled", true);
            }
            showPropzyAlert(response.message);
          });
        }
  ...
  relatedLsRequest: "",
  getRelatedListingRequest: function() {
      $("#relatedListingRequestModal").modal("show");
      try {
          lsDetail.relatedLsRequest.destroy();
      } catch (Ex) {}

      var lsoAjax = "/listing/getRelatedListingRequest?id=" + $("#rId").val();

      lsDetail.relatedLsRequest = $('#relatedListingRequest').on('processing.dt', function(e, settings, processing) {
          if (!processing) {
            hidePropzyLoading();
          } else {
            showPropzyLoading();
          ...
          "columns": [{
                  data: "requester",
                  render: lsDetail.renderRequester
              },
            //   {
            //       data: "rlistingId",
            //       render: lsDetail.renderRlistingId
            //   },
              // {
              //     data: "requester",
              //     render: lsDetail.renderRequester
              ...
              {
                  data: "content",
                  render: lsDetail.renderContent
              },
              {
                  data: "createdDate",
                  render: lsDetail.renderCreatedDate
              },
          ],
...
//   renderRequestedBy: function(data, type, object) {
//       return (object.requestedBy) ? object.requestedBy : "";
//   },
//   renderRlistingId: function(data, type, object) {
//       return (object.rlistingId) ? object.rlistingId : "";
//   },
  renderRequester: function(data, type, object) {
      return (object.requester) ? object.requester : "";
  },
  ...
  showRequestInternal: function() {
        showPropzyLoading();
        try {
            lsDetail.internalRequestLs.destroy();
        } catch (Ex) {}

        var lsoAjax = "/lso/get-internal-listings?rListingId=" + $("#rlistingId").val();

        lsDetail.internalRequestLs = $('#request-internal-listings').on('processing.dt', function(e, settings, processing) {
            if (!processing) {
              hidePropzyLoading();
            }
        }).DataTable({
            ...
            "columns": [{
                ...
                {
                    data: "creater",
                    render: lsDetail.renderCreater
                },
                {
                    data: "departmentName",
                    render: lsDetail.renderDepartmentName
                },
                {
                    data: "content",
                    render: lsDetail.renderContent
...
// Ngưng real listing
$("#finish-stop").click(function(e) {
    e.preventDefault();
    lsDetail.stopRealListing();
});

$("#stop-lso-submit").click(function(e) {
  e.preventDefault();
  lsDetail.stopListing();
});

// New feature 1/6/2017
$("#finish-sold").on('click', function (event) {
  event.preventDefault();
  lsDetail.soldLs();
});
...
$(".sold-by").change(function() {
  ...
  lsDetail.displaySoldOption(soldBy);
});

$("#sold-lso-submit").click(function(e) {
  e.preventDefault();
  lsDetail.soldListing();
});

$("#finish-rent").on("click", function (event) {
  event.preventDefault();
  lsDetail.rentLs();
});
...
$(".rent-by").change(function() {
  var rentBy = $(this).val();
  lsDetail.displayRentOption(rentBy);
});

$("#rent-lso-submit").click(function(e) {
  e.preventDefault();
  lsDetail.rentListing();
});

$("#showRequestFromLso").click(function(e) {
  e.preventDefault();
  lsDetail.getRelatedListingRequest();
});

$("#showInternalRequestFromLso").click(function(e) {
  e.preventDefault();
  $("#request-internal-modal").modal("show");
  lsDetail.showRequestInternal();
});
