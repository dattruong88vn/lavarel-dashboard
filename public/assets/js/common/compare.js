        ...
        if (compare.length >= 2) {
            ...
            App.Feature.Post("/api/detail-compare",data_send,function (data) {
                if (data == 'fail' || data == 'false') {
                    ...
                    $(Compare.selectedItems).each(function(index, item){
                        $("#check-compare-"+item).prop("checked", true);
                    });
                    if(App.UI.isMobile()){
...
$(function () {
        ...
        $(document).on('click','.compare_ids', function (e) {
            if($("input[name='check-compare']:checked").length > 0){
                $('#container-view-put').show();
            } else {
                $('#container-view-put').hide();
            ...
            $("input[name='check-compare']:checked").each(function(){
                Compare.selectedItems.push($(this).val());
            });
        });
        $('#time-put').datetimepicker({
        ...
        $(document).on('click', "#send-put",function () {
            ...
            $('#time-put').on('dp.change', function(e){
            ...
            if(checkValid) {
                ...
                postData.date = moment(checkValid.time.trim(), "DD/MM/YYYY HH:mm").unix() * 1000;
                ...
                var compare = Compare.getCompare();
                if (compare.length >= 0) {
                    $(".compare_ids:checked").each(function (idx, element) {
                        postData.rlistingIds.push(parseInt($(element).val()));
                        postData.links.push(location.protocol + "//" + location.host + $(element).data("link"));
                    });
                    postData.form_type = 13;
