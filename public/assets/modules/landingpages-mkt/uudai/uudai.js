var startApp = function () {
    ...
    App.UI.removeCheckSuccess(".form-uudai",['email']);
    ...
    $(".btn-request").click(function () {
        var postData = {};
        var curForm = $(this).closest(".form-uudai");
        if (!checkValidForm(curForm)) {
            App.UI.Error("Vui lòng nhập đầy đủ thông tin");
            return false;
        }
        $(".form-uudai").find("input, textarea").each(function (idx, element) {
        ...
        App.Feature.Post('/api/email-campaign', postData, function (data) {
            if (data.result) {
                ...
                $(".form-uudai input[type=text]").val('');
                $(".form-uudai input[type=email]").val('');
                $(".form-uudai .demand").val('');
