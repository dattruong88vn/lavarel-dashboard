...
var startApp = function(){
    ...
    App.UI.removeCheckSuccess(".form-infos",['email','phone']);
    // listing type
    $('#listingType').change(function(){
        ...
        if(stored.listingTypeId == 1) {
            // bán
            $('.text-price').html('Giá đề nghị');
            $('.col-deposit').hide();
            $('.propertyType-rent').hide();
            $('.propertyType-buy').show();
            $('.text-photoGcn').html('<span class="text">Bản vẽ/sổ </span>(tải lên tối thiểu 4 hình)<strong class="color-red">*</strong>');
            $("#price-button").html("<a href='javascript:;' data-target='#popup-require-price-buy' data-toggle='modal'>biểu phí giao dịch</a>");
        } else {
            // thue
            $('.text-price').html('Giá thuê (tháng)');
            $('.col-deposit').show();
            $('.propertyType-buy').hide();
            $('.propertyType-rent').show();
            $('.text-photoGcn').html('<span class="text">Bản vẽ/sổ</span>');
...
//        });
        ...
        if(houseNumber !== '' && fullAddress !== '' && ward !== '' && district !== ''){
            ...
            var getLongLat = function(){
                jQuery.ajax({
                    type: "GET",
                    dataType: "json",
                    url: "https://maps.googleapis.com/maps/api/geocode/json?ver=1.1&key=AIzaSyC5qvQV556mSpsvfIY1ew_PXGfYk9tAb-4",
    ...
    $("#btnSendAgent").click(function(){
        ...
        if($('.file-upload-preview-drawing').length < 4 && parseInt($("#listingType").val()) == 1){
        ...
        if(parseInt($("#listingType").val()) == 2){
            propertyTypeId = parseInt($("#propertyType_rent").val());
        }
        var postData = {
            "code": "",
            "sourceId" : 1,
            ...
            "info":{
                "listingTypeId": parseInt($("#listingType").val()),
    ...
    // 3. no login : sau khi bấm nút gửi thông tin sẽ hiện popup thông tin nhanh (như luồng cũ)
    ...
    if(typeof e !== "undefined") {
        $.ajax({
            ...
            success: function (result, textStatus, jqXHR) {
                ...
                if (obj.result) {
                    var src_img = obj.data.link;
                    html+= '<div class="div-col col-img">';
