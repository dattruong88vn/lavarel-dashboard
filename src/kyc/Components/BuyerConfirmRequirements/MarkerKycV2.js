...
class MarkerKyc extends React.Component{   
    ...
    render(){
        let that = this;
        if( (this.props.propertyTypeId == 8 || this.props.propertyTypeId == 1) && (that.props.filter.viewByWardId && that.props.filter.viewByWardId != null) )
        ...
        let labelStyleMarker = null; // label style
        let titleMarker = null; // title marker

        if(this.props.filter.viewByWardId) { // hiển thị listing
            labelStyleMarker = {textAlign:"center",backgroundColor: backgroundItem, color:"white", fontSize: "16px", padding: "7px"}; 
            titleMarker = `${this.props.item.formatPrice ? this.props.item.formatPrice : ""}`;
        }else{ // hiển thị group
            labelStyleMarker = {width:"50px",height:"50px",lineHeight:"50px",textAlign:"center",borderRadius:"50%",backgroundColor: "#1371a8", color:"white", fontSize: "12px", padding: "2px"}; 
            titleMarker = this.props.item.numRecord;
        }
        return (
            <Fragment>
                {titleMarker && <MarkerWithLabel
                            icon=" "
                            ...
                            labelStyle={labelStyleMarker}
                            onClick={() => {
                                $('.gm-ui-hover-effect').trigger("click");
                                if(that.props.filter.viewByWardId){
                                    showPropzyLoading();
                                    fetchListingDetailV2({listingId:that.props.item.listingId}, (response)=>{
                                    ...
                                    that.props.onFilterChange({
                                        ...
                                        "districtIds": that.props.item.districtId ? [that.props.item.districtId]: [],
                                        "wardIds": that.props.item.wardId ? [that.props.item.wardId]: []
                                    })
                                }
                            }}
                            >
                        ...
                        {that.props.filter.viewByWardId && <Marker 
                            icon=" "
                            position={{ lat: this.props.item.latitude, lng: this.props.item.longitude}} >
                                {that.state.isOpen && that.props.item.listingId == that.state.listingId && that.props.propertyTypeId != 1 && <InfoWindow position={{ lat: this.props.item.latitude, lng: this.props.item.longitude}} onCloseClick={() => {
                                    ...
                                    { that.state.listingDetail != null && <div style={{width:"300px"}}>
                                        ...
                                        <div style={{padding:"5px"}}>
                                            ...
                                            {(that.state.listingDetail.bpoLabel && that.state.listingDetail.bpoLabel !== 'N/A') && 
                                                <div style={{marginTop:"10px",marginBottom:"10px"}} className="row">
                                                    <div className="col-xs-4 col-md-4">
                                                        <strong>
                                                            {that.state.listingDetail.bpoLabel !== 'Đã BPO' ? 'BPO: ' : ''}
                                                            <span className={that.state.listingDetail.bpoLabel === 'Đã BPO' ? "btn btn-success btn-xs" : ''}>{that.state.listingDetail.bpoLabel}</span>
                                                        </strong>
                                                    </div>
                                                </div>
                                            }
                                            ...
                                            <div className="row">
                                                <div style={{textAlign:"center"}} className="col-md-12">
                                                    ...
                                                    postCreateCollection({
                                                        source:"kyc",
                                                        relatedListings:[this.props.item.listingId],
                                                        dealId:that.props.dealId,
                                                        searchScore:this.props.item.searchScore}, (response)=>{
                                                        if(response.result){
                                                            hidePropzyLoading();
