...
<div class='dashboard'>
    <section id="tasks" class="tm-home">
        <div class="content-header">
            <h2><a href="/">&larr;</a> 
                <?php echo $defineId == 32 ? "Chi tiết công việc" : "Meeting mới" ?>
            </h2>
        </div>
                ...
                <form id="formCreateMeeting" method="post" >
                    ...
                    @else
                        <div class="alert alert-warning">
                          <strong>Lưu ý!</strong> Vui lòng hoàn thành 1 hoặc tất cả bảng đánh giá khách hàng để tạo Deal.
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                ...
                                @foreach($task->meetingItem as $item)
                                    @if(!$item->isUpdateAfterView && $item->isGenerated)
                                        ...
                                        <a href="/lead/detail/{{$item->id->leadId}}">{{$item->id->leadId}}</a> 
                                        <span >: {{$item->request}}</span>
                                        <a title="Đánh giá khách hàng" data-toggle="tooltip" style="margin-right: 6%" href="#" class="btn btn-primary" onclick="openModalQuestion('{{$item->id->leadId}}', '2',null,'compare');return false;"><i class="fa fa-id-card-o"></i> </a>
                                    @else
                                        <a href="/deal/detail/{{$item->dealId}}">{{$item->dealId}}</a> 
                                        <span style="margin-right: 6%">: {{$item->request}}</span>
                                        <a title="Đã đánh giá khách hàng" data-toggle="tooltip" href="#" class="btn btn-primary disabled"><i class="fa fa-id-card-o"></i></a>
                                    @endif
                                    
                                @endforeach
                                
                                    @if($redirect && $defineId != 31)
            ...
            <div class="box-footer">
                <?php if (isset($task->isClosed) && $task->isClosed == false): ?>
                    <div class="form-group text-center">
                        ...
                        <?php if ($defineId != 31): ?>
                            <a href="#" class="btn btn-success makeCall" data-number="{{base64_encode($customerPhones)}}"><i class="fa fa-phone"></i> Gọi</a>
