        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- <div class="form-group row">
                    <div class="col-sm-12"><span class="required">*</span> bắt buộc nhập</div>
                    <div class="col-sm-12"><span class="required">Thang điểm </span> từ 1 đến <span class="lbConfigsCount"></span> và không được trùng nhau</div>
                </div> -->
...
<script type="text/template" class="config-new-listings-template">
    <div class='row config-{id}'>
        ...
        <div class='col-sm-9' ><input class='config-value form-control config-value-{id}' data-type-id='{id}' value='{value}' /></div>
        <div>{html}</div> -->
    </div>
</script>
<script>
    var ConfigNewListings = (function () {
        ...
        var showModal = function (dealId) {
            modalConfig = $("#modalConfigNewListings");
            var configTemplate = $(".config-new-listings-template").html();
            showPropzyLoading();
            var url = "/new-listings/get-config-data";
            if (dealId) {
                url += "?dealId=" + dealId;
                modalConfig.find(".dealId").val(dealId);
            } else {
                modalConfig.find(".dealId").val("");
            }
            $.ajax({
                "url": url,
                "type": "get"
            }).done(function (response) {
                // response = JSON.parse('{"result":true,"code":"200","message":"Thao tác thành công","validatedMessage":null,"data":[{"typeId":1,"name":"Giá","maxValue":8,"isUse":true,"control":"checkbox","valueInputText":null,"subCongif":[],"isPrimary":false},{"typeId":5,"name":"Quận","maxValue":8,"isUse":true,"control":"checkbox","valueInputText":null,"subCongif":[],"isPrimary":false},{"typeId":4,"name":"Phường","maxValue":3,"isUse":true,"control":"checkbox","valueInputText":null,"subCongif":[],"isPrimary":false},{"typeId":2,"name":"Hướng","maxValue":3,"isUse":true,"control":"checkbox","valueInputText":null,"subCongif":[],"isPrimary":false},{"typeId":3,"name":"Loại hẻm","maxValue":3,"isUse":true,"control":"checkbox","valueInputText":null,"subCongif":[{"typeId":6,"name":"Hẻm","maxValue":3,"isUse":true,"control":"radio","valueInputText":3.5,"subCongif":null,"isPrimary":false},{"typeId":7,"name":"Hẻm xe hơi","maxValue":3,"isUse":false,"control":"radio","valueInputText":null,"subCongif":null,"isPrimary":false}],"isPrimary":false}],"additional":null}');
                ...
                if (response.result) {
                    for (var i = 0; i < response.data.length; i++) {
                        ...
                        if (item.subCongif.length == 0) {
                            ...
                            if(item.typeId==3){
                                /* html = '<div class="row"><div class="col-md-7">' + radio + '</div><div class="col-md-5"><div class="checkbox"><label><input ' + disiable + ' type="checkbox"  value="">Chọn làm tiêu chí tính điểm</label></div></div></div>';*/
        ...
        var setUpButtonSave = function () {
            ...
            btnSave.on("click", function (event) {
                ...
                var postData = {
                    "dealId": modalConfig.find(".dealId").val(),
                    "values": []
                }
...
//                    if(jQuery.inArray( parseInt(value), validateDublicate ) != -1 && !$(this).prop('disabled') && $(this).parent('.col-md-7').siblings().find('input').is(":checked")){
//                        lbErrors.html("Tiêu chí tính điểm là số nguyên từ 3 đến 5 & không được trùng nhau");
//                        isValidate = false;
//                        return false;
//                    }
                    validateDublicate.push(parseInt(value));
                    // if (parseInt(value) != parseFloat(value)) {
                    //     lbErrors.html("Số nhập vào phải là số nguyên từ 1 đến " + configsCount);
                    //     isValidate = false;
                    //     return false;
                    // }
                    //
                    ...
                    totalScore += value;
                    var item = null;
                    // if (postData.dealId && postData.dealId.trim() != "") {
                    //     item = generateStaffValue(typeId, value);
                    // } else {
                    //     item = generateManagerValue(typeId, value);
                    // }
                    // console.log($(this).parent('.col-md-7').siblings().children('input').html());
                    //  "typeId": 6,
                    // "maxValue": 3,
                    // "isUse": true,
                    // "valueInputText": 3.5
                ...
                $('#modalConfigNewListings input[type=radio]').each(function (k, v) {
                    if ($(this).is(':checked')) {
                        // console.log($(this).parents('div').next('input').length);
                ...
                //     lbErrors.append("<br/>Giá trị nhập vào không được trùng nhau hoặc bỏ trống!");
                //     isValidate = false;
                // }
