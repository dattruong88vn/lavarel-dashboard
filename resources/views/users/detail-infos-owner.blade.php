...
<div class="detail-infos-owner-page">
    <div class="row">
        <div class="col-md-12 owner-details">
            <div class="col-md-12 owner-image">
                <input type="hidden" id="ownerId" value="{{ $ownerId }}"/>
                <?php if($detailInfos->photo): ?>
                    <img class="img-photo" src="{{ $detailInfos->photo }}" alt="" title="">
                <?php else: ?>
                    <img class="img-photo" src="/images/icon-12.png" alt="" title="">
                <?php endif ?>
                <p class="status-active"><i class="fa fa-circle"></i> Active</p>
                <p class="owner-name">{{ $detailInfos->name }}</p>
            </div>
        </div>
    </div>
    <div class="row body-infos-owner">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <div class="row col-md-12 head-detail">
                        ...
                        <select class="pull-right status-owner form-control" id="status-owner" name="status_owner">
                    ...
                    <div class="row info-head">
                        ...
                        <div class="row col-md-12">
                            ...
                            <div class="col-md-4">
                                <p>{{ date('d-m-Y', $detailInfos->lastActivityDate/1000) }}</p>
                            </div>
                        </div>
                        <div class="row col-md-12 title-col">
                            <div class="col-md-4">
                        ...
                        <div class="row col-md-12">
                            ...
                            <div class="col-md-4">
                                <p>{{ date('d-m-Y', $detailInfos->startDate/1000) }}</p>
                            </div>
                            <div class="col-md-4">
                                <p>chưa có</p>
                            </div>
                    ...
                    <div class="table-list">
                        ...
                        <div class="group-button-deal text-center">
                            @foreach ($buttonsPre as $button)
                                @if (!empty($button->childs))
                                    <div class="btn-group btn-see">
                                        <button onclick="showData('lso',{{ $ownerId }},{{ $button->id }});" type="button" class="btn btn-default">{{ $button->name }}</button>
                                        ...
                                        <ul class="dropdown-menu" role="menu">
                                            @foreach ($button->childs as $child_button)
                                                <li><a onclick="showData('lso',{{ $ownerId }},{{$button->id}}, {{ $child_button->id }});">{{ $child_button->name }}</a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                @else 
                            ...
                            @foreach ($buttonsSa as $button)
                                @if (!empty($button->childs))
                                    <div class="btn-group btn-see">
                                        <button onclick="showData('rlisting',{{ $ownerId }},{{$button->id }});" type="button" class="btn btn-default">{{ $button->name }}</button>
                                        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                            <span class="caret"></span>
                                            <span class="sr-only">Toggle Dropdown</span>
                                        </button>
                                        <ul class="dropdown-menu" role="menu">
                                            @foreach ($button->childs as $child_button)
                                                <li><a onclick="showData('rlisting',{{ $ownerId }},{{$button->id}}, {{ $child_button->id }});">{{ $child_button->name }}</a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                @else 
...
<!-- modal -->
<div id="updateInfosContacts" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content update-infos-contacts-owner">
            ...
            <div class="modal-body">
                <div id="contacts-infos" class="form-group row contacts-infos">
                    <form id="form-update-infos-owner">
                        <div class="col-md-4">
                            <?php if($detailInfos->photo): ?>
                                <img class="img-photo" src="{{ $detailInfos->photo }}" alt="" title="">
                            <?php else: ?>
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="form-row row">
                                ...
                                <input type="hidden" name="photo" id="photo" value="{{ $detailInfos->photo }}">
                            </div>
                            ...
                            <div class="form-row row">
                                <input id="name" name="name" class="form-control" type="text" value="{{ $detailInfos->name }}"/>
                            ...
                            @if(!empty($emails))
                                <div class="form-group wrapEmailProfile">
                                    ...
                                    @endforeach
                                    <a onclick="addMoreEmailOwnerProfile(this);return false;" href="#">Thêm email</a>
                                </div>
                            @else
                                <div class="form-group wrapEmailProfile">
                                    <div class="row">
                            ...
                            @if(!empty($phones))
                                <div class="form-group wrapPhoneProfile">
                                    ...
                                    @endforeach
                                    <a onclick="addMorePhoneOwnerProfile(this);return false;" href="#">Thêm số điện thoại</a>
                                </div>
                            @else
                                <div class="form-group wrapPhoneProfile">
                                    <div class="row">
                            ...
                            <div class="form-row row">
                                <input id="cmnd" name="cmnd" class="form-control" type="text" value="chưa có"/>
                                <div class="errors"></div>
                            </div>
                            <div class="form-row row">
                            ...
                            <div class="form-row row departmentIds">
                                <input id="address" name="address" class="form-control" type="text" value="chưa có"/>
                                <div class="errors"></div>
                            </div>
                            <div class="form-row row">
                            ...
                            <div class="form-row row">
                                <input id="startDate" name="startDate" class="form-control" type="text" value="{{ date('d-m-Y', $detailInfos->startDate/1000) }}"/>
                                <div class="errors"></div>
                            </div>
                        </div>
                    </form>
...
<script src="{{loadAsset("/js/user/detail-infos-owner.js")}}"></script>
<script type="text/javascript">
    ...
    function addMorePhoneOwnerProfile(element){
    ...
    function addMoreEmailOwnerProfile(element){
        let parent = $(element).parent('.form-group');
        let sibling = $(element).siblings('.row:first-child');
...
<link href="{{loadAsset("/css/user-role/detail-infos-owner.css")}}" rel="stylesheet" type="text/css" />
