...
<div class="detail-infos-agent-page">
    <div class="row">
        <div class="col-md-12 agent-details">
            <div class="col-md-12 agent-image">
                <input type="hidden" id="agentId" value="{{ $userInfor->agentId }}"/>
                ...
                <?php endif ?>
                <p class="status-active"><i class="fa fa-circle"></i> {{ $userInfor->statusName }} </p>
                <p class="agent-name">{{ $userInfor->name }}</p>
            </div>
        </div>
    </div>
    <div class="row body-infos-agent">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <div class="row col-md-12 head-detail">
                        ...
                        <select id="status-agent" class="pull-right status-agent form-control" name="status_agent">
                            <option {{ $userInfor->statusId == 1 ? 'selected' : ''}} value="1">Chưa training</option>
                            <option {{ $userInfor->statusId == 7 ? 'selected' : ''}} value="7">Đã training</option>
                            <option {{ $userInfor->statusId == 8 ? 'selected' : ''}} value="8">Reject</option>
                            <option {{ $userInfor->statusId == 9 ? 'selected' : ''}} value="9">Deactivate</option>
                            <option {{ $userInfor->statusId == 10 ? 'selected' : ''}} value="10">Mới tạo</option>
                            <option {{ $userInfor->statusId == 11 ? 'selected' : ''}} value="11">Hoàn thành</option>
                            <option {{ $userInfor->statusId == 12 ? 'selected' : ''}} value="12">Hủy</option>
                            <option {{ $userInfor->statusId == 13 ? 'selected' : ''}} value="13">Mới đăng ký</option>
                            <option {{ $userInfor->statusId == 14 ? 'selected' : ''}} value="14">Hoàn thành</option>
                            <option {{ $userInfor->statusId == 15 ? 'selected' : ''}} value="15">Học lại</option>
                            <option {{ $userInfor->statusId == 16 ? 'selected' : ''}} value="16">Hủy</option>
                            <option {{ $userInfor->statusId == 17 ? 'selected' : ''}} value="17">Đã ký</option>
                            <option {{ $userInfor->statusId == 18 ? 'selected' : ''}} value="18">Chưa ký</option>
                        </select>
                    </div>
                    ...
                    <div class="table-list">
                        ...
                        <div class="group-button-deal text-center">
                            <div class="group-button-deal text-center">
                                @foreach ($buttonsPre as $button)
                                    @if (!empty($button->childs))
                                        <div class="btn-group btn-see">
                                            <button onclick="showDataPosted('lso',{{ $agentId }},{{ $button->id }});" type="button" class="btn btn-default">{{ $button->name }}</button>
                                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                                <span class="caret"></span>
                                                <span class="sr-only">Toggle Dropdown</span>
                                            </button>
                                            <ul class="dropdown-menu" role="menu">
                                                @foreach ($button->childs as $child_button)
                                                    <li><a onclick="showDataPosted('lso',{{ $agentId }},{{$button->id}}, {{ $child_button->id }});">{{ $child_button->name }}</a></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    @else 
                                    ...
                                    @endif
                                @endforeach
                                @foreach ($buttonsSa as $button)
                                    @if (!empty($button->childs))
                                        <div class="btn-group btn-see">
                                            <button onclick="showDataPosted('rlisting',{{ $agentId }},{{$button->id }});" type="button" class="btn btn-default">{{ $button->name }}</button>
                                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                                <span class="caret"></span>
                                                <span class="sr-only">Toggle Dropdown</span>
                                            </button>
                                            <ul class="dropdown-menu" role="menu">
                                                @foreach ($button->childs as $child_button)
                                                    <li><a onclick="showDataPosted('rlisting',{{ $agentId }},{{$button->id}}, {{ $child_button->id }});">{{ $child_button->name }}</a></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    @else 
                    ...
                    <div class="box-body">
                        <table id="data-detail-posted" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>RID</th>
                                    <th>LID</th>
                            ...
                            <div class="group-button-deal text-center">
                                @foreach ($deal_buttons as $button)
                                    @if (!empty($button->childList) && $button->numberOfDeals !=0)
                                        <div class="btn-group btn-see">
                                            <button onclick="showData('{{$userInfor->agentId}}',{{$button->statusId }});" type="button" class="btn btn-default">{{ $button->statusName }} ({{ $button->numberOfDeals }})</button>
                                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                                <span class="caret"></span>
                                                <span class="sr-only">Toggle Dropdown</span>
                                            </button>
                                            <ul class="dropdown-menu" role="menu">
                                                @foreach ($button->childList as $child_button)
                                                    <li><a onclick="showData('{{$userInfor->agentId}}',{{$button->statusId}}, {{ $child_button->statusId }});" >{{ $child_button->statusName }} ({{ $child_button->numberOfDeals }})</a></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    @else 
...
<!-- modal -->
<div id="updateInfosContactsAgent" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content update-infos-contacts-agent">
            ...
            <div class="modal-body">
                <div id="contacts-infos" class="form-group row contacts-infos">
                    <form id="form-update-infos-agent">
                        <div class="col-md-3">
                            <?php if($userInfor->photo): ?>
                                <img class="img-photo" src="{{ $userInfor->photo }}" alt="" title="">
                            <?php else: ?>
                        ...
                        <div class="col-md-9">
                            ...
                            @if(count($emails) > 0)
                                <div class="form-group wrapEmailProfile">
                                    ...
                                    @endforeach
                                    <a onclick="addMoreEmailAgentProfile(this);return false;" href="#">Thêm email</a>
                                </div>
                            @else
                                <div class="form-group wrapEmailProfile">
                                    <div class="row">
                            ...
                            @if(count($phones) > 0)
                                <div class="form-group wrapPhoneProfile">
                                    ...
                                    @endforeach
                                    <a onclick="addMorePhoneAgentProfile(this);return false;" href="#">Thêm số điện thoại</a>
                                </div>
                            @else
                                <div class="form-group wrapPhoneProfile">
                                    <div class="row">
...
<script src="{{loadAsset("/js/user/detail-infos-agent.js")}}"></script>
<script type="text/javascript">
    ...
    function addMorePhoneAgentProfile(element){
    ...
    function addMoreEmailAgentProfile(element){
        let parent = $(element).parent('.form-group');
        let sibling = $(element).siblings('.row:first-child');
...
<link href="{{loadAsset("/css/user-role/detail-infos-agent.css")}}" rel="stylesheet" type="text/css" />
